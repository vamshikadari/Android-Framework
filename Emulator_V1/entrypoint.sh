...
if [ $(emulator -accel-check | awk 'NR==2{print $1}') == "0" ]; then

    echo "Using x86 architecture with $DEVICE"
	
	yes | sdkmanager "platforms;android-29" "system-images;android-29;google_apis;x86"

    echo "no" | avdmanager create avd -n emuTest -k "system-images;android-29;google_apis;x86" --device "$DEVICE"
	
else 

    echo "Using ARM architecture with $DEVICE"
	
	yes | sdkmanager "platforms;android-25" "system-images;android-25;google_apis;armeabi-v7a"

    echo "no" | avdmanager create avd -n emuTest -k "system-images;android-25;google_apis;armeabi-v7a" --device "$DEVICE"
